          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.map(event => (
              <div
                ...
                className={`px-4 py-3 border-b border-gray-800 cursor-pointer transition-colors hover:bg-gray-900 ${
                  selectedId === event.id ? 'bg-gray-900 border-l-2 border-l-white' : ''
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {event.magnitude && (
                    <span className={`text-xs px-1.5 py-0.5 rounded shrink-0 ${
                      event.magnitude >= 5 ? 'bg-red-900 text-red-300' :
                      event.magnitude >= 3.5 ? 'bg-orange-900 text-orange-300' :
                      'bg-yellow-900 text-yellow-300'
                    }`}>M{event.magnitude}</span>
                  )}
                  {event.category && (
                    <span className="text-xs px-1.5 py-0.5 rounded shrink-0 bg-blue-900 text-blue-300">{event.category}</span>
                  )}
                </div>
